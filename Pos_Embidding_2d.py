...
def get_position_angle_vec(position,d_pos_vec_v):
            return [position / np.power(10000, 2 * (hid_j // 2) / d_pos_vec_v) for hid_j in range(d_pos_vec_v)]
...
position_enc = np.array([
    [pos / np.power(10000, ((2*(j+1)))/d_pos_vec_v) for j in range(d_pos_vec_v)]
    if pos != 0 else np.zeros(d_pos_vec_v) for pos in range(n_position)])
position_enc[:, 0::2] = np.sin(-position_enc[:, 0::2]) # dim 3i
position_enc[:, 1::2] = np.cos(position_enc[:, 1::2]) # dim 3i+1
#position_enc[:, 2::3] = np.power(position_enc[:, 2::3],0) # dim 3i+2
# Apply the sine function to the even colums


position_enc_h[:, 0::2] = np.cos(position_enc_h[:, 0::2]) # dim 3i
position_enc_h[:, 1::2] = np.sin(position_enc_h[:, 1::2]) # dim 3i+1
...
#------------------------------------
for i in range(10):
    position_enc_v = position_enc_v + position_enc_h[i]
    
    k,j=i//4,i%4
    im = ax[k,j].imshow(position_enc_v[0:n_position], cmap='ocean', aspect='auto')
    #im = ax1.imshow(position_enc_v[0:40], cmap=cm.coolwarm, aspect='auto')
...
#------------------------------------
for i in range(10):
    position_enc_cube = position_enc_v + position_enc_h[i]
    position_enc_cube=np.linalg.norm(position_enc_cube[:, None, :] - position_enc_cube[None, :, :], axis=-1)
    k,j=i//4,i%4
    im = ax[k,j].imshow(position_enc_cube, cmap='ocean_r', aspect='auto')
    #im = ax1.imshow(position_enc_v[0:40], cmap=cm.coolwarm, aspect='auto')
    #ax[k,j].set_ylabel('\nPosition de mot + Position d\'énoncé '+str(i),fontsize=8)
    #ax[k,j].set_xlabel('Dimension de plongement',fontsize=8)
    
    ax[k,j].set_ylabel('\nDistance entre deux Position'+str(i),fontsize=7)
    ax[k,j].set_xlabel('\nDistance entre deux Position',fontsize=7)
    #ax[k,j].set_title(r'Word position + utterance position '+str(i),fontsize=10)
    fig.colorbar(im, shrink=1,  ax=ax[k,j])
# Tweak spacing to prevent clipping of ylabel
fig.tight_layout()
